...
COMSCORE.SiteRecruit.Utils.getPageWidth = function(hAlign) {
	switch (hAlign) {
		case 0: 
			return "0%";
			break;
...
COMSCORE.SiteRecruit.Utils.getPageHeight = function(vAlign) {
	switch (vAlign) {
			case 0: 
			return "0%";
			break;
...
COMSCORE.SiteRecruit.Invitation = {
	METHODOLOGY: { STANDARD: 0, EMAIL: 1, DD: 2, QINVITE: 3, CDDS: 4, CLE: 5 },
...
COMSCORE.SiteRecruit.Invitation.Renderer = ( function() {
	...
	return {
		...
		init: function() {
			...
			switch (c.horizontalAlignment) {
				...
				case this.HORIZONTAL_ALIGNMENT.MIDDLE:
					_lastX = _utils.getPageWidth(1);
					break;
					
				case this.HORIZONTAL_ALIGNMENT.RIGHT:
					_lastX = _utils.getPageWidth(2);
					break;
			...
			switch (c.verticalAlignment) {
				...
				case this.VERTICAL_ALIGNMENT.MIDDLE:
					_lastY = _utils.getPageHeight(1);
					break;
					
				case this.VERTICAL_ALIGNMENT.BOTTOM:
					_lastY = _utils.getPageHeight(2);
					break;
					
				
			}

			this.setPosition(_lastX, _lastY, _width, _height);
		},
		...
		start: function(invitation) {
			...
			// Show gray mask if it exists
			if (document.getElementById("grayMask")) {
				document.getElementById("grayMask").style.visibility = "visible";
			}
		...
		destroy: function() {
			...
			// Remove gray mask
			if (this.invitation.config.grayMask) {
				this.invitation.config.gm.style.visibility = "hidden";
				removeEvents(this.invitation.config.grayMask);
			}
...
COMSCORE.SiteRecruit.Invitation.BaseInvitation.prototype = {
	...
	logView: function() {
		...
		params += "&location=" + escape(COMSCORE.SiteRecruit.Utils.location)
			...
			params += "&c4=1";
			params += "&c5=" + COMSCORE.SiteRecruit.device.type;
			params += "&c6=" + COMSCORE.SiteRecruit.Builder.invitation.config.methodology;
	...
	preRender: function() {
		...
		if (this.config.closeWithEsc && this.config.closeWithEsc == true) {
	...
	render: function() {
		...
		if (this.config.isAutoCentering == true) {
			s.position = "fixed";
		}
		else {
			s.position = "absolute";
		...
		// Create gray mask
		if (this.config.grayMask) {
			this.config.gm = document.createElement("div");
			this.config.gm.id = "grayMask";
			this.config.gm.style.visibility = "hidden";
			this.config.gm.style.position = "fixed";
			this.config.gm.style.top = "0px";
			this.config.gm.style.left = "0px";
			this.config.gm.style.padding = "0px";
			this.config.gm.style.width = "100%";
			this.config.gm.style.height = "100%";
			
			this.config.gm.style.backgroundColor = "#333333";
			this.config.gm.style.zIndex = 10000;
			this.config.gm.style.opacity = 7/10;
			this.config.gm.style.filter = 'alpha(opacity=' + 70 + ')';
			
			document.body.appendChild(this.config.gm);
		}
...
COMSCORE.SiteRecruit.Utils.lang.extend(COMSCORE.SiteRecruit.Invitation.DD, COMSCORE.SiteRecruit.Invitation.BaseInvitation, {
		accept: function() {
			...
			if (document.location.protocol == "https:") 
			{
				if (COMSCORE.SiteRecruit.Broker && COMSCORE.SiteRecruit.Broker.config && COMSCORE.SiteRecruit.Broker.config.tracker && COMSCORE.SiteRecruit.Broker.config.tracker.ssl && COMSCORE.SiteRecruit.Broker.config.tracker.ssl != '') {
					url = this.appendSurveyDataToAcceptUrl(COMSCORE.SiteRecruit.Broker.config.tracker.ssl);
				}
				else {
					url = this.appendSurveyDataToAcceptUrl(COMSCORE.SiteRecruit.executingPath + this.config.url);
				}
			}
			else
			{
				if (COMSCORE.SiteRecruit.Broker && COMSCORE.SiteRecruit.Broker.config && COMSCORE.SiteRecruit.Broker.config.tracker && COMSCORE.SiteRecruit.Broker.config.tracker.std && COMSCORE.SiteRecruit.Broker.config.tracker.std != '') {
					url = this.appendSurveyDataToAcceptUrl(COMSCORE.SiteRecruit.Broker.config.tracker.std);
				}
				else {
					url = this.appendSurveyDataToAcceptUrl(COMSCORE.SiteRecruit.executingPath + this.config.url);
				}
	...
	return {
		...
		createInvitationClass: function(config) {
			...
			switch (config.methodology) {
				...
				case _sr.Invitation.METHODOLOGY.CDDS:
					return new _sr.Invitation.DD(config);
		...
		chooseInvitation: function() {
			...
			var tstVal = _utils.UserPersistence.getCookieValue("tstMode");

			if (invs) {
				if (invs.length > 1 && /p\d+/i.test(tstVal) ) {
					for (i = invs.length - 1; i >= 0; i--) {
						var inv = invs[i];
						
						if (inv.projectId == tstVal) {
							this.invitation = this.createInvitationClass(invs[i]);
							return true;
						}	
					}
				}
